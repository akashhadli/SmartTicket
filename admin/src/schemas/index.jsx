...
export const operRegisterSchema = Yup.object({
	OperName: Yup.string()
		...
		.max(45, 'Name must be at max 25 characters')
		.matches(
			/[^\s*].*[^\s*]/g,
			'* This field cannot contain white space and special character'
		)
		.required('Please enter company name'),
	OperShortName: Yup.string()
		.min(2, 'Name must be at least 2 characters')
	...
	OperEmail: Yup.string()
		.email('Enter valid email')
		.required('Please enter company email'),
	OperPhone: Yup.string()
		.matches(phoneRegExp, 'Phone number is not valid')
		.max(10, 'Phone number must be equal to 10')
		.required('Please enter your phone number'),
	OperGSTIN: Yup.string()
		.max(15, 'GST no must be 15 characters')
		.matches(
			/^[a-zA-Z0-9]+$/,
			'* This field cannot contain white space and special character'
		)
	...
	OperPincode: Yup.string()
		.max(6, 'Pincode must be equal to 6.')
		.required('Please enter pincode'),
	OperContactName: Yup.string()
		.matches(
			/[^\s*].*[^\s*]/g,
			'* This field cannot contain white space and special character'
		)
...
export const empRegisterSchema = Yup.object({
	EmpName: Yup.string()
		...
		.required('Please enter empolyee name'),
	EmpIntId: Yup.string()
		.min(4, 'Name must be at least 4 characters')
	...
	EmpAadhar: Yup.string()
		.matches(aadharRegExp, 'Aadhar number is not valid')
		.max(12, 'Aadhar number must be equal to 12')
